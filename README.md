...
発生・状況の更新だけでなく、復旧したときにも通知します。
通知内容は `./notice_message.json` にも出力します。

- 標準ライブラリのみで動作します（外部依存パッケージなし。Go 1.26 以降を想定）
...
| ドキュメント | 内容 |
| --- | --- |
| [実行方法とテスト](docs/usage.md) | 実行コマンド、オプション、dry-run、テスト通知、テスト |
| [セットアップ](docs/setup.md) | Raspberry Pi / Linux への導入。Go のインストールからビルドまで |
| [設定](docs/configuration.md) | `.env` の書き方、設定値の優先順位、データ取得元の切り替え |
| [通知](docs/notification.md) | Discord / Slack への通知内容（新規・更新・復旧）、重複通知の抑止 |
| [出力ファイル](docs/output.md) | `notice_message.json` とログファイルの仕様 |
| [定期実行](docs/cron.md) | cron に登録して定期実行する手順 |
