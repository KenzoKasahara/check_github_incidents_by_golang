...
cron に登録して、定期的にインシデントを確認して通知する手順です。
先に [セットアップ](setup.md) を終えて、手動で動くところまで確認しておいてください。
...
[セットアップ](setup.md#3-バイナリをビルドする) のビルドがまだか、置き場所が違います。
別の場所に置きたい場合は `BIN=/path/to/check-github-incidents` で指定できます。
...
まず `logs/log-YYYYMMDD.log` の「前回の確認時点」の行を確認してください。

- `前回の確認時点の記録がありません。` と出ている場合は、`notified_incidents.json` を消した直後か
  初回実行です。過去の障害がまとめて飛ぶのを避けるため、直近 24 時間以内に解決したものだけを通知します
- `前回の確認時点: <日時>` と出ているのに通知が来ない場合は、その日時より後に解決したインシデントが
  無いか、すでに通知済み（`resolved_notified` に記録あり）です
- 記録が更新されるのは、実際に送信した実行だけです。`-dry-run` や Webhook URL 未設定のままでは
  目印が残らないため、いつまでも「前回の確認時点の記録がありません。」のままになります

検出の仕組みは [通知](notification.md#復旧の検出) を参照してください。
